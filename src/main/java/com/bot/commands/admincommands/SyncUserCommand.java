...
public class SyncUserCommand implements ICommand
{
    ...
    public void execute(String[] args, MessageReceivedEvent event) 
    {
        ...
        {
            log.warn("El comando !sync solo puede ser usado en un servidor.");
            return;
        }

        if (!event.getAuthor().getId().equals(_ownerId))
        {
            log.warn("Comando !sync denegado para el usuario: {}", event.getAuthor().getName());
            return;
        }
    ...
    {
        log.info("========== INICIO SYNC DE USUARIOS ==========");
        log.info("Servidor: {} ({})", server.getName(), server.getId());
        log.info("Total miembros: {}", total);
        log.info("============================================");
    }

    private void logEnd() 
    {
        log.info("=========== FIN SYNC DE USUARIOS ===========");
    }

    private void logError(Throwable error) 
    {
        log.error("Error al cargar los miembros del servidor: {}", error.getMessage());
    }
